...
authors: [flashbots]
tags: [research, privacy, block-building]
image: /img/posts/network-anonymized-mempools/image.png
hide_table_of_contents: false
description: Introducing Flashnet, a low-latency anonymous broadcast protocol for censorship resistance, privacy, and more permissionless block building.
---
...
![A look into Ethereum's execution pipeline from orderflow.art](/img/posts/network-anonymized-mempools/image.png)
...
Adding these guarantees is not purely attractive because they align with cypherpunk ideals. As we explain in our Builder Decentralization Roadmap, we see **strong privacy tooling as a necessary condition for the success of DeFi**. The execution path of DeFi transactions typically involves many actors - order flow auctions, solvers, relays, builders, validators - and execution quality depends on interactions between them. As we develop our ability to share sensitive information without risking abuse, blockchain execution pipelines will be able to incorporate more counterparties, more strategies and more information, benefiting both decentralization and execution quality.

So far we have only taken the first step. **Even when infrastructure runs in TEEs, operators are still able to learn metadata** opening up surface area for selective censorship and adversarial MEV strategies. Existing anonymity solutions, such as TOR and Nym, require users to choose between weak anonymity or high latency. Similarly, FOCIL promises to add censorship resistance but only at the cost of higher latency and required use of the public mempool which invites frontrunning and other undesirable forms of MEV. These tradeoffs can be impractical for a lot of use cases.
...
In the ideal world, we would like anonymity to be maximized, while latency and bandwidth overheads are at a minimum. In reality, however, there is no free lunch and often one has to pay a price in at least one metric. After all, what is a blockchain post without a trilemma?

![Trilemma overview](/img/posts/network-anonymized-mempools/image-2.png)
...
<div style={{textAlign: "center"}}><img src="/img/posts/network-anonymized-mempools/Trilemma.png" alt="Trilemma chart" style={{width: "55%"}} /></div>
...
![User-facing example of Flashnet flow.](/img/posts/network-anonymized-mempools/anon-comm.svg)
